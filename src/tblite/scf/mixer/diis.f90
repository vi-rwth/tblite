module tblite_scf_mixer_diis
    ...
    subroutine build_F(B, n, it, omit, saved_F, sol, F, info)
        ...
        real(wp), intent(in) :: saved_F(it*n**2)
        ...
        if (info==0) then
            F = 0.0_wp
            do i=1,it-omit-1
                F = F + sol(i,1)*saved_F((i+omit-1)*n**2+1:(i+omit)*n**2)
            end do
        end if
    ...
    subroutine build_B(err_v, sv_err, n, it, omit, old_B, B)
        ...
        if (it /= 1) then
            ...
            do i=1,it-omit-1
                call frobenius(sv_err((i+omit-1)*(n**2+n)/2+1:(i+omit)*(n**2+n)/2), err_v, Tr)
                B(it-omit, i) = Tr
                B(i, it-omit) = Tr
            end do
        end if
    ...
    subroutine save_vectors(F, err_v, n, it, old_sv_F, old_sv_err, sv_F, sv_err)
        ...
        real(wp), intent(in) :: F(n*n), err_v((n**2+n)/2)
        real(wp), intent(in) :: old_sv_F((it-1)*n**2), old_sv_err((it-1)*(n**2+n)/2)
        real(wp), intent(out) :: sv_F(it*n**2), sv_err(it*(n**2+n)/2)
        if (it/=1) then
            sv_err(1:(it-1)*(n**2+n)/2) = old_sv_err
            sv_F(1:(it-1)*n**2) = old_sv_F
        end if          
        sv_err((it-1)*(n**2+n)/2+1:it*(n**2+n)/2) = err_v
        sv_F((it-1)*n**2+1:it*n**2) = F
    ...
    subroutine diis(n, it, F, e_max, error_v, memory, info)
        ...
        integer, intent(out) :: info
        real(wp), intent(inout) :: F(n*n), e_max
